...
public class ActionField extends JPanel {
	...
	public void clean() throws Exception {
		...
		int separator = battleField.getQuadrant(defender.getX(), defender.getY()).indexOf(
				"_");
		defender.moveToQuadrant(1, 1);
		int v = Integer.parseInt(battleField.getQuadrant(defender.getX(), defender.getY())
				.substring(0, separator));
		int u = Integer.parseInt(battleField.getQuadrant(defender.getX(), defender.getY())
	...
	private boolean processInterception() {
		...
		if (fireTank(v, u,
				battleField.getQuadrant(agressor.getX(), agressor.getY()))) {
			agressor.destroy();
			return true;
		}
		if (v >= 0 && v < 9 && u >= 0 && u < 9) {
			if (battleField.scanQuadrant(v, u).equals("")
					...
					battleField.scanQuadrant(v, u).equals("W")) {
				return false;
			} else {
				
					if (battleField.scanQuadrant(v, u).equals("B")) {
						battleField.getBrick(v, u).destroy();
					}
					if (battleField.scanQuadrant(v, u).equals("E")) {
						battleField.getEagle().destroy();
				}
					if (battleField.scanQuadrant(v, u).equals("R")) {
				    if (defender instanceof Tiger) {
						battleField.getRock(v, u).destroy();
					}
					}
			}
		}
			...
			return false;
		if (fireTank(v, u,
				battleField.getQuadrant(defender.getX(), defender.getY()))) {
			defender.destroy();
		}
		return true;
	}
	...
	public ActionField() throws Exception {
		...
		battleField.setElements();
		defender = new T34(this, battleField);
		bullet = new Bullet(-100, -100, Direction.NONE);
		String location = battleField.getAgressorLocation();
		agressor = new Tiger(this, battleField, Integer.parseInt(location
				.split("_")[1]), Integer.parseInt(location.split("_")[0]),
				Direction.DOWN);
	...
	protected void paintComponent(Graphics g) {
		...
		for (int j = 0; j < battleField.getBlength(); j++)
			battleField.getBricks()[j].draw(g);
		for (int j = 0; j < battleField.getRlength(); j++)
			battleField.getRocks()[j].draw(g);
		for (int j = 0; j < battleField.getWlength(); j++)
			battleField.getWaters()[j].draw(g);
		battleField.getEagle().draw(g);
